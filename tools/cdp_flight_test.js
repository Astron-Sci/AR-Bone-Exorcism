// cdp_flight_test.js — 抛射飞行中截图（骨头+拖尾）
...
const ud = mkdtempSync(join(tmpdir(), 'bonex-fl-'));
...
(async () => {
  ...
  await sleep(450);
  await shot('dbg_flight.png');
  const st = await ev(`window.__boneGame.flying ? 'FLYING t=' + window.__boneGame.flying.t.toFixed(2) : 'landed'`);
  console.log('飞行状态:', st);
  ws.close();
