// cdp_matrix_dbg2.js — 局部矩阵 vs 世界矩阵
...
const ud = mkdtempSync(join(tmpdir(), 'bonex-mx2-'));
...
(async () => {
  ...
  const out = await ev(`(() => {
    ...
    const fmt = m => Array.from(m.elements).map(v => +v.toFixed(2));
    const before = fmt(s.matrix);
    s.updateMatrix();  // 强制重算局部矩阵
    const after = fmt(s.matrix);
    s.updateMatrixWorld(true);
    const world = fmt(s.matrixWorld);
    return JSON.stringify({ before, after, world, sceneChildren: window.__dbg.scene.children.map(c => c.name || c.type) });
  })()`);
  console.log(out);
  ws.close();
