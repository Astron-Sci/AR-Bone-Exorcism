// cdp_orient_test.js — 模型朝向测试（+90° vs -90°）
...
const ud = mkdtempSync(join(tmpdir(), 'bonex-or-'));
...
(async () => {
  ...
  await sleep(2500); // 等 scale 收敛
  await ev(`window.__dbg.skeleton.rotation.y = Math.PI/2; true`);
  await shot('dbg_orient_p90.png');
  await ev(`window.__dbg.skeleton.rotation.y = -Math.PI/2; true`);
  await shot('dbg_orient_n90.png');
  ws.close();
