// cdp_miss_test.js — 验证抛掷失败路径（miss → 回骨袋）
...
const ud = mkdtempSync(join(tmpdir(), 'bonex-ms-'));
...
(async () => {
  ...
  await sleep(800);
  /* 大幅偏移 → 必 miss */
  await ev(`window.__boneGame.throwNow({x:1.2,y:1.4}); true`);
  await sleep(2200);
  console.log('抛1(大幅偏移) lastResult:', await ev(`JSON.stringify(window.__boneGame.lastResult)`));
  console.log('state:', await ev(`window.__boneGame.state`), '(3=附身,5=抛掷中) | 第1块done:', await ev(`window.__boneGame.missing[0].done`));
  /* demo 模式 miss 后应自动重新进入抛掷 */
  console.log('miss后 currentBone:', await ev(`window.__boneGame.currentBone`));
  ws.close();
