// cdp_orient_geo2.js — 验证 YXZ 组合旋转后的朝向与肩宽
...
const ud = mkdtempSync(join(tmpdir(), 'bonex-ogeo2-'));
...
(async () => {
  ...
  const check = await ev(`(() => {
    ...
    const w = name => {
      ...
      return { x: +e[12].toFixed(4), y: +e[13].toFixed(4), z: +e[14].toFixed(4) };
    };
    const ls = w('left_scapula'), rs = w('right_scapula');
    const lz = w('left_zygomatic_bone'), rz = w('right_zygomatic_bone');
    const head = w('frontal_bone'), occ = w('occipital_bone');
    const rot = { order: skeleton.rotation.order, x: +skeleton.rotation.x.toFixed(3), y: +skeleton.rotation.y.toFixed(3) };
    return JSON.stringify({
      rot,
      shoulderWidthX: ls && rs ? Math.abs(ls.x - rs.x).toFixed(4) : null,
      shoulderDepthZ: ls && rs ? Math.abs(ls.z - rs.z).toFixed(4) : null,
      zygomaZ: { left: lz && lz.z, right: rz && rz.z },
      frontalZ: head && head.z, occipitalZ: occ && occ.z,
    });
  })()`);
  console.log(check);
  ws.close();
